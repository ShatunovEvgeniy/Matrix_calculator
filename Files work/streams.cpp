...
void calculate(const string& name, const string& out_name) // choose an operation and write result
{
    ...
    vector<string> znaki {"+", "-", "*", "/"};   // possible operations with two matrixes
    ...
    while (true)
    {
        ...
        if (in_available(aviable, operation))     // if the operation is unary
        {
        ...
        else if (in_available(znaki, operation))  // if the operation is binarys
        {
